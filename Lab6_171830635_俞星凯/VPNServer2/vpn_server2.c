...
void readRouteTable(){
    ...
    while(!feof(fp)){
        ...
        printf("*** Get server2 vpnEntrance : %s ***\n",printIP(vpnEntrance));

        memcpy(vpnExit,exit,20);
        printf("*** Get server2 vpnExit     : %s    ***\n", vpnExit);
    ...
    if(ethPack->ethType == htons(ETH_P_ARP)){
        ...
        if(i == routeItemNum){
            ...
            if( routeTable[i].ifIndex  == 2 ){
                char tmp[6]={0x00,0x0c,0x29,0x22,0x58,0x56};//network eth1
    ...
    if(ethPack->ethType == htons(ETH_P_ARP)){
        ...
        if(i == routeItemNum){
            ...
            if( routeTable[i].ifIndex  == 3 ){
                char tmp[6]={0x00,0x0c,0x29,0x10,0x64,0xb3};//pc2 macAddr
                memcpy(dstAddr.sll_addr,tmp,ETH_ALEN);
